...
\noindent\textbf{Bottleneck Profile by Compression Ratio. }Compression and decompression dominate the total execution time for high-ratio circuits, shifting the primary bottleneck from physical I/O to the CPU; dense circuits exhibit the inverse profile, retaining I/O as the dominant cost. For 34Q GHZ ($R=255\times$), physical I/O operations (\texttt{pread} and \texttt{pwrite}) account for 1.3s, representing 2.1\% of the 63.3s total execution time. Conversely, for 34Q VQC ($R=2.16\times$), physical I/O reaches 534.6s, representing 62.3\% of the 858.2s total execution time. This contrast demonstrates that only high compression ratios (e.g., GHZ's 255$\times$) eliminate the NVMe bandwidth constraint, while dense ratios (e.g., VQC's 2.16$\times$) leave the storage path active. The system transitions between CPU-bound and I/O-bound regimes depending on per-circuit compression, validating the design trade-off introduced in Section~\ref{design_compression}.
