...
\begin{table}[t]
\centering
\caption{State fidelity $F = |\langle\psi_\text{exact}|\psi_\text{lossy}\rangle|^2$ at depth 5 and maximum qubit count reachable in the dual-file ping-pong layout within the 500\,GB SSD across truncation depths. Bold $T=8$ columns mark the chosen operating depth.}
...
For all circuits up to 30 qubits (where verification is feasible), Table~\ref{tab:fidelity} reports the measured state fidelity and the maximum qubit count reachable on the 500\,GB SSD across truncation depths $T \in \{10, 8, 6, 4\}$ at depth 5. At the chosen $T=8$ operating point, fidelity ranges from 0.9919 (QV) to 0.9998 (GHZ), maintaining $F \geq 0.99$ across the six circuits, and the ping-pong layout supports 39Q for sparse and Clifford-structured circuits (GHZ, QSVM, QV) and 36Q for dense circuits (Random, VQE, VQC). Lower $T$ values trade fidelity for compression ($T=6$ drops dense circuits to 0.9453--0.9544 and $T=4$ further to 0.78--0.95), while $T=10$ raises all circuits above 0.9957 but caps QV at 38Q and VQC at 35Q within the 500\,GB capacity (Figure~\ref{fig:eff_ratio}).
