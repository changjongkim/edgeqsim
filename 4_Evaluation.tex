...
\noindent\textbf{Storage Footprint Reduction.} Figure~\ref{fig:eff_footprint} compares logical state vector size with the on-disk footprint of the dual-file ping-pong layout. As qubit count grows from 34Q to 39Q, the logical footprint expands from 128 GB to 4 TB, while the ping-pong footprint scales from 1.00 GB to 32.13 GB. Each backing file occupies half of this footprint, corresponding to a per-file LZ4 compression ratio of 254$\times$ to 256$\times$ across all qubit counts. This ratio indicates stable compression efficiency as the state vector grows. The dual-file layout doubles on-disk usage to ensure atomic per-layer updates, yielding an effective storage reduction of 128$\times$ relative to the logical size. Note that this 128$\times$ metric reflects on-disk footprint with both backing files coexisting, whereas the per-direction I/O volume during execution scales with a single file at the full 256$\times$ per-file ratio. As a result, \EdgeQuantum fits a 39-qubit simulation within 32 GB of NVMe storage, whereas a raw representation exceeds capacity by 2$\times$ at 36Q and 16$\times$ at 39Q.
...
\EdgeQuantum reaches a 4~TB state vector simulation (39 qubits) on the 8~GB Jetson, surpassing the 256 GB SSD limit by 16$\times$ via LZ4 ping-pong compression. Beyond the 34Q regime where the raw state vector fits within the 256 GB SSD, simulating 35 to 39 qubits requires the LZ4 ping-pong compression layer because the logical footprint reaches 1$\times$ to 16$\times$ the physical storage capacity. It is necessary to evaluate whether \EdgeQuantum can sustain stable execution in this regime, where alternative simulators are infeasible. Table~\ref{tab:extreme_results} reports the per-circuit simulation time of \EdgeQuantum at depth 5 across 35Q (256 GB) to 39Q (4 TB).
\EdgeQuantum sustains stable execution across the 35 to 39Q range, with simulation time scaling near-linearly as the state vector doubles at each qubit increment. For compute-dense circuits (Random, QV, VQC, VQE), runtime grows from approximately 590s at 35Q to 9,864s at 39Q, a 16.7$\times$ increase across a 16$\times$ state vector growth. For example, Random scales from 590.79s (35Q) to 9,864.12s (39Q), and VQC reaches 9,912.18s ($\approx$2.75 hours) at 39Q. Lower-density circuits complete the 39Q simulation faster. QSVM completes in 3,960.33s and GHZ in 1,980.20s. The near 2$\times$ time growth per qubit confirms that the ping-pong pipeline retains compression and decompression efficiency as workload scales by 16$\times$, with no degradation from queue contention or fragmentation.
