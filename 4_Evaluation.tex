...
Additionally, we confine \textit{BMQSim-like} to 30-34Q because 1) at 35Q, \EdgeQuantum requires compression to fit the 500 GB SSD, conflating I/O scheduling with compression contribution, and 2) single-file in-place rewrite is structurally incompatible with async read-write overlap, forcing \textit{BMQSim-like} into the same blocking pattern already characterized at 30-34Q.
...
\noindent\textbf{Read.} Across all configurations, \EdgeQuantum reduces read latency by 1.47$\times$ to 1.75$\times$, with compute-dense circuits at the higher end of the range. For example, at 32Q VQC, the read time drops from 142.3s to 81.3s (1.75$\times$). GHZ performs minimal computation and still achieves 1.47$\times$ to 1.62$\times$ read speedup. In contrast, \textit{BMQSim-like} issues concurrent reads and writes that contend within the NVMe controller queue, preventing either direction from sustaining the sequential bandwidth measured in Section~\ref{back2}. By blocking new reads until the preceding write completes (Section~\ref{design_pipeline}) and streaming chunks via \texttt{O\_DIRECT} directly into UVM ring buffers without the intermediate \texttt{cudaMemcpy} required by \textit{BMQSim-like} (Section~\ref{design_memory}), \EdgeQuantum avoids this contention and sustains the 1.38 GB/s sequential read bandwidth.
