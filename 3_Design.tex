...
\begin{figure}[t]
    ...
    \caption{Data path comparison. (Top) Existing execution requires explicit \texttt{cudaMemcpy}, doubling memory usage and causing severe bandwidth bottlenecks. (Bottom) \EdgeQuantum leverages UVM and \texttt{O\_DIRECT} to achieve zero-copy execution on UMA, transferring ownership via MMU pointer switches without physical data migration.}
    \label{uvm_diff}
\end{figure}
...
\noindent\textbf{Cross-Chunk Gate Handling.}
