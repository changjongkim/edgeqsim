...
In this paper, we propose \EdgeQuantum, a scalable tiered-memory simulation framework for full state vector quantum simulation on resource-constrained edge devices. By coordinating UMA-aware zero-copy data movement with asynchronous I/O serialization and mantissa-truncated LZ4 ping-pong compression, we mask NVMe bottlenecks and extend simulation capacity beyond physical DRAM. We evaluate \EdgeQuantum on an 8~GB NVIDIA Jetson Orin Nano, achieving up to 4.59$\times$ speedup over blocking I/O baselines and supporting full state vector simulation up to 39 qubits (4~TB), a 512$\times$ logical capacity expansion beyond DRAM.
