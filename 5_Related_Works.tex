...
\noindent\textbf{Memory Expansion and I/O Optimization Strategies.} Previous studies on memory expansion focus on extending simulation capacity by offloading state vectors to high-performance NVMe SSDs in server environments~\cite{park2022snuqs, zhang2025bmqsim}. These approaches rely on OS-level demand paging or explicit memory mapping. Other work proposes lossless compression and adaptive encoding to reduce memory consumption~\cite{zhang2025bmqsim, wu2018memory, wu2019full, song2023efficient}. These methods trade CPU cycles for capacity. In the broader context of deep learning, approaches such as vDNN and FlashNeuron simplify memory management by offloading intermediate feature maps to backing stores.

While our study aligns with these offloading and compression techniques, generic strategies remain inefficient on edge devices due to the bandwidth asymmetry of consumer-grade SSDs and UMA constraints. Prior server-centric frameworks primarily target high-end infrastructure and do not address the variable-size update constraints of commodity edge storage. \EdgeQuantum employs an asynchronous I/O strategy that serializes read and write operations to avoid NVMe contention. This is a critical optimization rarely considered in server-centric storage engines. Combined with UVM-aware zero-copy buffering and a dual-file ping-pong mantissa-truncated LZ4 compression strategy, \EdgeQuantum improves pipeline efficiency and minimizes CPU-GPU synchronization overhead.
