...
\label{sec:related}
\noindent\textbf{Optimizing Quantum Circuit Simulation.} Previous studies on quantum circuit simulation have focused on utilizing massive parallelism in GPUs and distributed clusters for full state vector simulation. These approaches accelerate execution by distributing the state across aggregated VRAM in leadership-class supercomputers. Other studies propose static compilation and circuit partitioning to reduce communication overhead. These methods maximize kernel occupancy through precompiled kernels and optimized execution plans. Additionally, tensor network-based approaches reduce memory footprint by decomposing quantum states into interconnected tensors. These approaches provide savings for low-entanglement circuits while facing exponential complexity growth in deep, highly entangled circuits.
...
\noindent\textbf{Memory Expansion and I/O Optimization Strategies.} Previous studies on memory expansion focus on extending simulation capacity by offloading state vectors to high-performance NVMe SSDs in server environments. These approaches rely on OS-level demand paging or explicit memory mapping. Other work proposes lossless compression and adaptive encoding to reduce memory consumption. These methods trade CPU cycles for capacity. In the broader context of deep learning, approaches such as vDNN and FlashNeuron simplify memory management by offloading intermediate feature maps to backing stores.
