...
Quantum circuit simulation involves trade-offs between scalability and computational complexity. Tensor network contraction reduces memory usage, but its computational cost grows exponentially for highly entangled circuits. In contrast, full state vector simulation explicitly maintains every probability amplitude, providing exact results regardless of circuit depth or entanglement structure~\cite{fu2024achieving, pan2022simulation, park2022snuqs, xu2024atlas}. To process these vectors, simulators such as \textit{cuStateVec} and \textit{Qiskit Aer} rely on GPU acceleration. They allocate the state vector in VRAM and process the circuit layer by layer, where CUDA kernels apply quantum gates in parallel, and data remains on-device, avoiding intermediate transfers.
...
\noindent\textbf{Per-iteration VQE Latency. }As shown in Figure~\ref{intro_motivation_b}, this memory wall manifests as the qubit count grows on edge devices. CPU-only execution (\textit{Cirq}) avoids GPU workspace overhead but pays orders-of-magnitude compute cost, requiring over 100 seconds for a single 29-qubit VQE iteration. GPU-accelerated execution (\textit{cuQuantum}) fails to simulate 29 qubits, immediately crashing with an Out-of-Memory (OOM) error. To address this, out-of-core memory management is required. Existing frameworks employ data compression and tiered memory pipelines to mitigate memory limits. However, these approaches are optimized for server-grade GPUs with discrete PCIe topologies and abundant host resources. Adapting them to edge environments requires falling back to blocking I/O on external NVMe storage. Their pipelines assume host-driven DMA paths that are absent on UMA SoCs, stalling GPU computation and incurring severe I/O latency.
...
\begin{figure}[t]
    ...
    \begin{subfigure}[b]{\columnwidth}
        \centering
        \includegraphics[width=0.8\linewidth]{Figures/edge_back2.png}
    ...
    \begin{subfigure}[b]{\columnwidth}
        \centering
        \includegraphics[width=0.89\linewidth]{Figures/edge_back3.png}
