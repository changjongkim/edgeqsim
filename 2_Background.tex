...
\begin{figure}[t]
    ...
    \caption{Bandwidth across the edge memory hierarchy on the Jetson Orin Nano. Unified DRAM bandwidth is measured via CUDA memory copy operations. External NVMe SSD bandwidth is evaluated using FIO benchmark~\cite{axboe-fio} with \texttt{O\_DIRECT} (256~KB blocks for sequential and 4~KB blocks for random accesses).}
